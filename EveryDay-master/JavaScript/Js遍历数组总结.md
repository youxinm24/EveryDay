...
const copiedArr = arr.map(arr => arr);
...
- `forEach`：用于执行副作用，返回 `undefined`。
- `map`：用于生成新的数组，返回一个新的数组，常用于变换或复制数组。
...
//对于数组，for...in 的遍历顺序不一定是按索引顺序的，因为 for...in 只遍历属性（索引是数组的属性），它不保证顺序。另外，如果数组上有非整数键或自定义属性，它们也会被遍历。
...
const greaterThan2 = arr.filter(item => item > 2); //简写省略 返回的return
